    ...
    superlu_dist_options_t* options)
{
    ...
    //
        ...
        //malloc
    cuDoubleComplex* d_nzval, * d_tempv;
    int* d_nzvals, * d_segment_offset, * d_segment_ptr;
    checkGPU(cudaMalloc((void**)&d_nzval, sizeof(cuDoubleComplex) * ldv * nsupc));
    checkGPU(cudaMalloc((void**)&d_tempv, sizeof(cuDoubleComplex) * nbrow * nsupc));
    checkGPU(cudaMalloc((void**)&d_nzvals, sizeof(int) * max_supernode_size));
    checkGPU(cudaMalloc((void**)&d_segment_offset, sizeof(int) * segment_compress.segment_count));
    checkGPU(cudaMalloc((void**)&d_segment_ptr, sizeof(int) * (segment_compress.segment_count + 1)));

    zcatter_l_compress(non_zero_col_num, nbrow, max_supernode_size, nsupc, usub,
        klst, iukp, nzval, ldv,
        tempv,
        segment_compress, d_nzval, d_segment_ptr,
        d_segment_offset, d_tempv, d_nzvals, temp_nbrow);
